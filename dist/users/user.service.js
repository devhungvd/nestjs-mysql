...
let UserService = class UserService {
    ...
    async findOneById(id) {
        ...
        if (!user) {
            throw new apollo_server_express_1.UserInputError(`User #${id} not found`);
        }
        return user;
    }
    async create(createUserInput) {
        createUserInput.password = bcrypt.hashSync(createUserInput.password, 8);
        const user = this.userRepository.create(Object.assign({}, createUserInput));
